...
def temperature(i, j, t) :
    return T0 + deltaT*((i - 32)**2 + (j - 32)**2 <= R**2)*np.exp(-((t-t0)**2)/sigma**2)

def MapSpin(spin, T) :
    plt.figure(facecolor='white', figsize=(8,8), dpi=72)
    grid=spin[1:-1,1:-1]
    ...
    plt.savefig('gravure_T-{}.png'.format(T))
#---------------------------------------------
...
#paramètres pour afficher la simulation---------------------------------------
color = mpl.colors.ListedColormap(['r','b'])
bounds=[-2,0,2]
norme = mpl.colors.BoundaryNorm(bounds,color.N)
#-----------------------------------------------------------------------------
...
t0 = 100
temp = temperature(32, 32, space)
plt.plot(space, temp/np.max(temp), 'k--')
